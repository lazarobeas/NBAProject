...
filepath = 'C:\\Users\\Lazaro B\\Documents\\GitHub\\NBAProject\\data\\JamalMurray\\JamalMurrayGameEntireCareer.csv'
...
plt.ylabel('True')
plt.title(f'Confusion Matrix - Jamal Murray Simulated with {point_threshold} points')
plt.show()

# Input the feature values for today's game
print("Enter Jamal Murray's stats for the last 5 games:\n")
today_game = {
...
if today_game_prediction[0] == 1:
    print(f"\nThe model predicts that Jamal Murray will score over {point_threshold} points in today's game.")
else:
    print(f"\nThe model predicts that Jamal Murray will score under {point_threshold} points in today's game.")