...
filepath = 'C:\\Users\\Lazaro B\\Documents\\GitHub\\NBAProject\\data\\CalebMartin\\CalebMartinGameEntireCareer.csv'
...
plt.ylabel('True')
plt.title(f'Confusion Matrix - Caleb Martin Simulated with {point_threshold} points')
plt.show()

# Input the feature values for today's game
print("Enter Caleb Martin's stats for the last 5 games:\n")
today_game = {
...
if today_game_prediction[0] == 1:
    print(f"\nThe model predicts that Caleb Martin will score over {point_threshold} points in today's game.")
else:
    print(f"\nThe model predicts that Caleb Martin will score under {point_threshold} points in today's game.")