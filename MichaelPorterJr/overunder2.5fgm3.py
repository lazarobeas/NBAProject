...
features = ['HOME', 'AST', 'STL', 'REB', 'TOV', 'FG3A', 'PTS', 'BLK']
target = 'FG3M'

X = data[features]
y = data[target]

# Binarize the target variable based on the threshold of 2.5 FG3M
...
plt.ylabel('True')
plt.title('Confusion Matrix - Will Michael Porter Jr get Over 2.5 FG3M?')
plt.show()
...
today_game = {
    ...
    'FG3A': 5.6,  # Average made 3-point field goals
...
if today_game_prediction[0] == 1:
    print("The model predicts that Michael Porter Jr will score over 2.5 FG3M in today's game.")
else:
    print("The model predicts that Michael Porter Jr will score under 2.5 FG3M in today's game.")